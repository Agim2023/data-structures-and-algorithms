...
const htmlListItems = transformToLis(inputObject);
...
const count = (target, input) => {
  ...
  return input.reduce((totalCount, currentArray) => {
    const countInCurrentArray = currentArray.reduce((arrayCount, num) => {
      if (num === target) {
        return arrayCount + 1;
      }
      return arrayCount;
    }, 0);
    return totalCount + countInCurrentArray;
  }, 0);
};

const targetNumber = 5;
const inputArrays = [[1, 3, 5, 7, 9], [5, 5, 5], [1, 2, 3]];
const result = count(targetNumber, inputArrays);
console.log(result);
